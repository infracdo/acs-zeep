<template>
  ...
  >
      ...
      <v-toolbar flat>
            ...
            <v-form ref="form" v-model="valid" lazy-validation>
              <v-card-text>
                <v-row no-gutters>
                  <v-col cols="1" md="1">
                    <v-subheader
                      ><div v-html="'Serial Number <strong>*</strong>'"></div
                    ></v-subheader>
                  </v-col>
                  <v-col cols="4" md="4">
                    <v-text-field
                  ...
                  <v-col cols="1" md="1">
                    <v-subheader
                      ><div v-html="'Parent Group <strong>*</strong>'"></div
                    ></v-subheader>
                  </v-col>
                  <v-col cols="4" md="4">
                    <v-autocomplete
                ...
                <v-row no-gutters>
                  <v-col cols="1" md="1">
                    <v-subheader
                      ><div v-html="'Device Alias <strong>*</strong>'"></div
                    ></v-subheader>
                  </v-col>
                  <v-col cols="4" md="4">
                    <v-text-field
                  ...
                  <v-col cols="1" md="1">
                    <v-subheader
                      ><div v-html="'Device Type <strong>*</strong>'"></div
                    ></v-subheader>
                  </v-col>
                  <v-col cols="4" md="4">
                    <v-select
        ...
        <v-dialog v-model="Multi_dialogDelete" max-width="500px">
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue darken-1" text @click="Multi_closeDelete"
                >Cancel</v-btn
              >
              <v-btn color="blue darken-1" text @click="Multi_deleteItemConfirm"
                >OK</v-btn
              >
              <v-spacer></v-spacer>
            </v-card-actions>
    ...
    <template v-slot:[`item.status`]="{ item }">
      ...
      <v-icon small v-if="getColor(item.status) == 'white'">
...
export default {
  ...
  data: () => ({
    ...
    serial_list: [],
    device: [],
    items: [{ title: "restart" }, { title: "move" }, { title: "delete" }],
    editedIndex: -1,
    editedItem: {
      id: "",
      status: "offline",
  ...
  computed: {
    ...
    computedHeaders() {
      var i,
        returnHeaders = new Array();
      returnHeaders = this.headers;
      for (i in this.filterableHeaders) {
        if (!this.filterableHeaders[i].show)
          returnHeaders = returnHeaders.filter(
            (headers) => headers.text !== this.filterableHeaders[i].text
          );
      }
  ...
  methods: {
      ...
      http
        ...
        .then((response) => {
          ...
          for (i in response.data) {
            ...
            if (response.data[i].parent == "unassigned")
              this.device.push(response.data[i]);
          }
    ...
    initGroup() {
      ...
      this.serialRules.push(
        (v) => this.serialList.indexOf(v) < 0 || "Serial already exist"
      );
    },
    ...
    moveConfirm() {
      ...
      for (i in this.selected) {
        ...
        if (this.device.indexOf(this.selected[i]) != -1)
          this.device[this.device.indexOf(this.selected[i])].parent =
            this.editedItem.parent;
        this.editedItem = Object.assign(
          {},
          this.device[this.device.indexOf(this.selected[i])]
        );
        http
    ...
    close() {
      if (
        (!this.editedItem.parent && !this.editedItem.device_name) ||
        this.isSave
      ) {
